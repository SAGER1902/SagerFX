...
import tgfx.packets.PacketEntityDeathType;
import tgfx.packets.PacketSpawnParticle;
import tgfx.packets.PacketSpawnParticleOnEntity;
...
public class TGPackets implements ITGInitializer {
	...
	public static EntityPlayer getPlayerFromContext(MessageContext ctx){
		EntityPlayer thePlayer = (ctx.side.isClient() ? TechgunsFX.proxy.getPlayerClient() : ctx.getServerHandler().player);
		return thePlayer;
	}
	...
	public void init(FMLInitializationEvent event) {
		...
		network = NetworkRegistry.INSTANCE.newSimpleChannel(TechgunsFX.MODID);
		int packetid=0;
